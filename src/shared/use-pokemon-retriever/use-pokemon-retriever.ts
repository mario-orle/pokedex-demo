...
function setup(): UsePokemonRetriever {
  ...
  async function getByName(name: string): Promise<Pokemon> {
    return new Promise((res, rej) => {
      addPriorityRequest<Pokemon>(`${apiBaseUrl}/pokemon/${name}`, pokemonResponse => {
  ...
  function pokemonListRetriever(url = `${apiBaseUrl}/pokemon?limit=${defaultPageSize}`): void {
    addPriorityRequest<Paginated<ListPokemon>>(url, async (paginatedPokemons) => {
      paginatedPokemons.results.map(partialPokemon => pokemons.value.push({name: partialPokemon.name}));
  ...
  function backgroundEnrichment(): void {
    const batch = pokemons.value.filter(pokemon => !pokemon.requested && !pokemon.id);
    batch.forEach(pokemon => {
      pokemon.requested = true;
      addRequest<Pokemon>(`${apiBaseUrl}/pokemon/${pokemon.name}`, enrichPokemon);
    })
  }

  function enrichPokemon(pokemonResponse: Pokemon) {
    const idx = pokemons.value.findIndex(p => p.name === pokemonResponse.name);
    pokemons.value[idx] = {...pokemons.value[idx], ...pokemonResponse, requested: true };
  }
